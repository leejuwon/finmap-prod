...
- DSR/LTV common `tool_calculate`: PASS
...
| --- | --- | --- | --- |
| `tool_calculate` | `pages/tools/*`, `_components/DsrLtvCalculator.js` | User submits or updates a calculator and a result is produced | `source_tool`, `locale`, `currency`, `has_result`, `location` |
...
- `_components/DsrLtvCalculator.js` sends the common `tool_calculate` event with `source_tool: "dsrLtv"`, `currency: "KRW"`, `has_result: true`, `location: "form_submit"`.
- `_components/DsrLtvCalculator.js` also keeps `dsr_ltv_calculate` with `source_tool`, `locale`, `interaction`, `has_result` as a DSR/LTV-specific detailed analysis event.
...
| `location` | CTA events | Usually `result_cta`, `sticky_cta`, or `form_submit`. |
...
DSR/LTV is now included in the same common funnel through `tool_calculate` with `source_tool = dsrLtv`. Keep `dsr_ltv_calculate` separately for DSR/LTV-specific interaction analysis such as `input_change` and `preset`.
...
| `node scripts\verify_tool_result_cta_events.js` | PASS. All required events and parameters found; DSR/LTV common `tool_calculate` found; existing `dsr_ltv_calculate` kept; raw email not found in GA4/localStorage/CustomEvent payloads. |
