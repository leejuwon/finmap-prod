// pages/market/real-estate/songpa-top100.js
...
const REGION = {
  ...
  lawd: "11710", // 서울 송파구
  nameKo: "송파구(잠실 포함)",
  nameEn: "Songpa-gu (incl. Jamsil)",
  detailSido: "11",
};

const TEXT = {
  ko: {
    title: "송파(잠실) 아파트 집값 TOP 100 | 송파구 실거래 순위",
    desc:
    ...
    faqs: [
      {
        q: "이 페이지의 ‘송파(잠실)’은 어디 범위인가요?",
        a: "행정구역 기준 서울 송파구(11710) 전체를 포함합니다. ‘잠실’은 송파구 내 주요 지역이라 검색어 관점에서 함께 표기했습니다.",
      },
      {
        q: "순위 기준은 무엇인가요?",
      ...
      {
        q: "평형/년식을 고정해서 보고 싶어요.",
        a: "대시보드로 이동해서 평형/년식/금액구간/정렬을 추가로 설정해 더 공정하게 비교할 수 있습니다.",
      },
    ],
  },
  en: {
    title: "Songpa (Jamsil) Apartment Prices Top 100 | Official Transactions",
    desc:
    ...
    faqs: [
      {
        q: "What is the scope of this page?",
        a: "Administrative scope is Songpa-gu (11710). Jamsil is included as it’s within Songpa-gu.",
      },
...
const SEO = {
  ...
  keywordsKo: "송파 아파트 집값, 잠실 아파트 집값, 송파 아파트값 순위, 실거래 Top100",
  keywordsEn: "Songpa apartment prices, Jamsil apartment prices, Seoul real estate ranking, top 100",
};
...
export default function SongpaTop100Page({ period, band, rows }) {
  return (
...
export async function getServerSideProps(ctx) {
  ...
  const sql = `
    ...
    LIMIT 100
  `;
  const [rows] = await pool.query(sql, [period, band, REGION.lawd]);
