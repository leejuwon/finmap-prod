const { simulateRetirementPlan } = require("./retirementCore");
...
export function mapFireParamsToRetirementInput(params = {}, contributionYears = 0) {
  const tax = (Number(params.taxRatePct) || 0) / 100;
  const fee = Number(params.feeRatePct) || 0;
  const annualReturnPct = Number(params.annualReturnPct) || 0;
  const netAnnualReturnPct = (annualReturnPct - fee) * (1 - tax);
  const years = Math.max(0, Number(contributionYears) || 0);

  return {
    currentAge: 0,
    retirementAge: years,
    lifeExpectancy: years + 60,
    currentAssets: Number(params.currentAsset) || 0,
    monthlySaving:
      (Number(params.monthlyContribution) || 0) +
      (Number(params.annualContribution) || 0) / 12,
    annualReturn: netAnnualReturnPct,
    retirementReturn: netAnnualReturnPct,
    inflation: Number(params.inflationPct) || 0,
    monthlyExpense: (Number(params.annualSpending) || 0) / 12,
  };
}
...
export function runFireSimulation(params = {}) {
  ...
  const retirementCore = simulateRetirementPlan(
    mapFireParamsToRetirementInput(params, contributionYears)
  );
