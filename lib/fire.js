...
export function runFireSimulation(params) {
  ...
  // 단순 모델: 명목 = 실질 + 인플레이션
  const netNominal = netReal + inf;

  // -----------------------------
  // 1) 적립 구간 시뮬레이션
  // -----------------------------
  const years = Number(accumulationYears) || 0;
  let assetReal = Number(currentAsset) || 0;
  let assetNominal = Number(currentAsset) || 0;
  ...
  for (let y = 1; y <= years; y++) {
    const startReal = assetReal;
    const startNominal = assetNominal;

    // 적립 후
    const midReal = startReal + yearlySave;
    const midNominal = startNominal + yearlySave;

    // 수익 적용
    const endReal = midReal * (1 + netReal);
    const endNominal = midNominal * (1 + netNominal);

    assetReal = endReal;
    assetNominal = endNominal;

    const reached = fireTarget > 0 && endReal >= fireTarget;
    if (reached && fireYear === null) {
      fireYear = y;
    }
  ...
  const canFireAtEnd = fireTarget > 0 && retirementStartReal >= fireTarget;

  // -----------------------------
  // 3) 은퇴 구간 시뮬레이션 (실질 + 명목 모두)
  // -----------------------------
  let retireReal = retirementStartReal;
  let retireNominal = retirementStartNominal;
  const spending = Number(annualSpending) || 0;
  ...
  for (let y = 1; y <= 60; y++) {
    const startReal = retireReal;
    const startNominal = retireNominal;

    const afterWithdrawReal = startReal - spending;
    const afterWithdrawNominal = startNominal - spending;

    if (afterWithdrawReal <= 0) {
      retRows.push({
        year: y,
        phase: "retirement",
        assetReal: 0,
        assetNominal: Math.max(afterWithdrawNominal, 0),
        withdrawal: spending,
        depleted: true,
        fireTarget,
      });
    ...
    const endReal = afterWithdrawReal * (1 + netReal);
    const endNominal = afterWithdrawNominal * (1 + netNominal);

    retireReal = endReal;
    retireNominal = endNominal;
  ...
  const timeline = [
    ...accRows.map((r) => ({
      year: r.year,
      phase: r.phase,
      assetReal: r.assetReal,
      assetNominal: r.assetNominal,
