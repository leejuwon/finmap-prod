 ...
 * @param {string} params.taxMode  - 'apply' | 'none'
 * @param {string} params.feeMode  - 'apply' | 'none'
 */
export function calcCagr({
  ...
  const taxRate = taxMode === 'apply' ? 0.154 : 0; // 15.4% 이자소득세
  const feeRate = feeMode === 'apply' ? 0.005 : 0; // 연 0.5% 수수료 가정

  // 아주 단순화된 근사:
  //   net ≈ gross * (1 - taxRate) - feeRate
  // → gross ≈ (net + feeRate) / (1 - taxRate)
  let grossCagr = (netCagr + feeRate) / (1 - taxRate || 1);
